        ...
        <div className="p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 border border-gray-300 text-gray-700 px-6 py-3 rounded-xl font-semibold hover:bg-gray-50 transition-all"
            >
              <Copy size={18} />
              Salin Nomor Tiket
            </button>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6">
              ...
              {showCalendar && (
                <div className="absolute top-12 left-0 z-50 bg-white border border-gray-200 rounded-2xl shadow-xl p-4 w-72">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <button
                      onClick={goPrevMonth}
                      className="w-7 h-7 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-600"
                    >
                      ‹
                    </button>
                  ...
                  <div className="grid grid-cols-7 gap-0.5">
                    {Array.from({
                      length: getFirstDay(calMonth, calYear),
                    }).map((_, i) => (
                      <div key={i} />
                    ))}
                    {Array.from({
                      length: getDaysInMonth(calMonth, calYear),
                    }).map((_, i) => {
                      const day = i + 1;
                      const today = getToday();
                      const isToday =
                      ...
                      const selM = tanggal
                        ? parseInt(tanggal.split("/")[1]) - 1
                        : null;
                      const selY = tanggal ? parseInt(tanggal.split("/")[2]) : null;
                      const isSelected =
                        selD === day && selM === calMonth && selY === calYear;
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <div className="flex-1">
              <p className="text-xs text-gray-400 mb-2">
                Upload foto, dokumen, atau screenshot sebagai bukti
              </p>
              <div
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mt-6 flex items-center gap-2 text-sm text-blue-700">
          ...
          <span>
            Laporan Anda bersifat <strong>anonim</strong>. Data pribadi tidak
            perlu diisi.
          </span>
        </div>

        {error && (
          <p className="text-red-500 text-sm mt-4 font-medium bg-red-50 border border-red-200 rounded-xl p-3">
            {error}
          </p>
        )}
