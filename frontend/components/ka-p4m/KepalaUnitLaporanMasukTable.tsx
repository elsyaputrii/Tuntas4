...
> = {
  menunggu_review: {
    label: "Menunggu Review Staf P4M",
    cls: "text-blue-500 bg-blue-50 border-blue-200",
    Icon: Clock,
  },
  disetujui: {
    label: "Disetujui",
          ...
          return (
            <div key={item.id_boxing} className={`${idx > 0 ? "border-t-2 border-black" : ""}`}>
              ...
              <div className="sm:hidden p-4 space-y-2">
                ...
                <p className="text-[10px] text-gray-500 flex items-center gap-1">
                  <Target size={11} />
                  Target Selesai: {item.tanggal_rencana ? fmtDate(item.tanggal_rencana) : "—"}
                </p>
                {badge && (
                  <div className={`px-2 py-1 border rounded text-[10px] font-medium inline-flex items-center gap-1 ${badge.cls}`}>
                    <badge.Icon size={11} />
                    {badge.label}
                  </div>
                )}
              ...
              <div className="hidden sm:flex min-h-32">
                ...
                <div className="w-[20%] border-r-2 border-black p-5">
                  ...
                  <p className="mt-1 text-[10px] text-gray-500 flex items-center gap-1">
                    <Target size={10} />
                    Target: {item.tanggal_rencana ? fmtDate(item.tanggal_rencana) : "—"}
                  </p>
                </div>
                <div className="flex-1 p-5 flex items-center justify-center">
                  {badge ? (
                    <div className={`px-2 py-1 border rounded text-[10px] font-medium text-center inline-flex items-center gap-1 ${badge.cls}`}>
                      <badge.Icon size={11} />
                      {badge.label}
                    </div>
                  ) : (
