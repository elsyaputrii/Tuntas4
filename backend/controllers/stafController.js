...
//   8. setApprovalStaf      → Staf P4M setujui/tolak hasil tindak lanjut unit + catatan
//
// ════════════════════════════════════════════════════════════════════════
// PERBAIKAN TERBARU:
//   ✅ setApprovalStaf sekarang menerima parameter 'catatan' (wajib)
//      untuk menyimpan alasan Staf P4M menerima/menolak hasil unit.
//      Catatan disimpan di kolom 'catatan_approval' tabel boxing_ketidaksesuaian.
//   ✅ Validasi: catatan wajib diisi, tidak boleh kosong.
//   ✅ "diterima" → status_boxing = 'selesai' + approval_staf = 'diterima'
//   ✅ "ditolak" → status_boxing tetap 'di_staff' + approval_staf = 'ditolak'
//   ✅ Keduanya menyimpan catatan.
    ...
    try {
      ...
      await conn.query(
        `UPDATE boxing_ketidaksesuaian
         SET status = 'terdistribusi', approval_staf = 'menunggu', catatan_approval = NULL
         WHERE id_boxing = ?`,
        [id_boxing]
      );
...
// ============================================================
// 8. APPROVAL STAF + CATATAN — PATCH /api/staf/approval-boxing
// ============================================================
async function setApprovalStaf(req, res) {
  ...
  if (!catatan || catatan.trim() === "") {
    return res.status(400).json({
      success: false,
      message: "Catatan / alasan keputusan wajib diisi.",
    });
  }
  ...
  try {
    ...
    if (approval === "diterima") {
      // ✅ Simpan catatan + set selesai
      await pool.query(
        `UPDATE boxing_ketidaksesuaian 
         SET approval_staf = ?, status = 'selesai', catatan_approval = ?
         WHERE id_boxing = ?`,
        [approval, catatan.trim(), id_boxing]
      );
      await syncStatusLaporan(row.id_laporan);
    } else {
      // ✅ Simpan catatan + tetap di di_staff (revisi)
      await pool.query(
        `UPDATE boxing_ketidaksesuaian 
    ...
    return res.status(200).json({
      success: true,
      message: pesan,
      data: { id_boxing, approval, catatan: catatan.trim() }
    });
  } catch (error) {
